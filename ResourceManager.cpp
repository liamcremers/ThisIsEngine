...
auto dae::ResourceManager::LoadTexture(const std::string& file)
    -> dae::Texture2D*
{
    const auto fullPath = m_dataPath / file;
    const auto filename = fs::path(fullPath).filename().string();
    if (m_loadedTextures.find(filename) == m_loadedTextures.end())
        m_loadedTextures.insert(std::pair(
            filename, std::make_unique<Texture2D>(fullPath.string())));
    return m_loadedTextures.at(filename).get();
}

auto dae::ResourceManager::LoadFont(const std::string& file, uint8_t size)
    -> dae::Font&
{
    const auto fullPath = m_dataPath / file;
    const auto filename = fs::path(fullPath).filename().string();
    ...
    if (m_loadedFonts.find(key) == m_loadedFonts.end())
        m_loadedFonts.insert(
            std::pair(key, std::make_unique<Font>(fullPath.string(), size)));
